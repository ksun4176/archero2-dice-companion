          ...
          {Array.from({ length: numBreakpoints }, (_, index) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-xs font-bold text-success">+{reward}d</span>
              </div>
            )
          })}
        </div>
...
type QuestProgress = { [name in QuestName]: number }
...
export default function QuestTrackerContent() {
  ...
  const { numEarned, numRemaining } = useMemo(() => {
    ...
    Quests.forEach((quest) => {
      const numBreakpoints = quest.breakpoints[0].length;
      let numBreakpointsMet = quest.breakpoints[0].findIndex(bp => bp > questProgress[quest.name]);
      numBreakpointsMet = numBreakpointsMet === -1 ? numBreakpoints : numBreakpointsMet;
